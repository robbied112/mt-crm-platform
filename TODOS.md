...
> **Eng review refinements (2026-03-16):** Base64 callable (not Storage). Single Claude call with all pages (not per-page). Separate extractWines callable (not inside rebuildViews). New functions/billback.js (not in index.js monolith). Shared deduplicateEntities() helper (TODO-039). Vintage IS wine identity. Conditional DataContext loading behind feature flag. BillbackReviewStep sub-component in DataImport.

### TODO-032: PDF Billback Extraction Cloud Function
...
### TODO-033: Wine/Product Entity with AI Dedup
- **What:** New `wines/{wineId}` Firestore collection under each tenant. Auto-extract wine names from billback line items on import. `extractWines` is a SEPARATE callable Cloud Function (not inside rebuildViews — failure isolation). Uses shared `deduplicateEntities()` helper (TODO-039). Normalize → exact match → Claude fuzzy match → create new / link / queue for review. Schema: `{ name, normalizedName, displayName, producer, vintage, varietal, sku, region, sourceNames[], firstSeen, lastSeen, importIds[], metadata: {} }`. **Vintage IS identity** — "Margaux 2018" ≠ "Margaux 2019". Wine normalization: strip accents (château→chateau), remove bottle sizes (750ml), keep vintage years, handle abbreviations (Ch.→Château, Dom.→Domaine). Confidence thresholds: >0.85 auto-link, 0.5-0.85 pending review, <0.5 create new. New `pendingWineMatches/` subcollection for review queue. Add Firestore security rules for `wines/` and `pendingWineMatches/` (same pattern as accounts/).
- **Why:** Wine-level tracking IS the value proposition. Without a wine entity, can't deduplicate "Château Margaux 750ml" vs "Chateau Margaux 2018 750" vs "MARG 750" across billbacks from different distributors. Also becomes the entity that connects billback spend to depletion volume (cross-source intelligence).
...
- **Cons:** New entity = new CRUD, new Firestore security rules, new UI pages. AI dedup cost (~$0.01/import). Wine names are messier than account names (vintages, abbreviations, foreign characters like château/côtes).
...
- **Depends on:** TODO-032 (PDF extraction provides wine names), TODO-039 (shared dedup helper), TODO-023 (extractAccounts must exist to refactor into shared helper)

### TODO-034: Billback Transforms + Spend Views
...
- **Depends on:** TODO-032, TODO-022 (rebuildViews)

### TODO-035: Billback UI (DataImport Extension + Dashboard + Routes)
...
- **Depends on:** TODO-032 (extraction), TODO-034 (views), TODO-033 (wines)

### TODO-039: Extract shared deduplicateEntities() helper
...
### TODO-036: Agreement Management + Auto-Reconciliation
...
- **Depends on:** TODO-032, TODO-033, TODO-034, TODO-035 (all Phase 1 billback)

---

## P3 — Billback Phase 3: Intelligence

### TODO-037: Producer Allocation Dashboard
...
- **Depends on:** TODO-033 (wines), TODO-034 (spend views), ideally TODO-022 (depletion views for ROI)

### TODO-038: Predictive Spend Budgeting
...
- **Depends on:** TODO-034 (spend views), 6+ months of billback history
...
TODO-020 (shared package)
    ...
    │       │       └── TODO-034 (billback transforms + spend views)
    │       │               │
    │       │               ├── TODO-038 (predictive spend budgeting) ← Phase 3
    │       │               └── TODO-035 (billback UI) ← also needs TODO-032, TODO-033
    │       │                       │
    │       │                       └── TODO-036 (agreement mgmt + reconciliation) ← Phase 2
    │       │
    ...
    │       │       └── TODO-039 (extract shared deduplicateEntities helper)
    │       │               │
    │       │               └── TODO-033 (wine entity + AI dedup) ← uses shared helper
    │       │                       │
    │       │                       └── TODO-037 (producer allocation dashboard) ← Phase 3
    │       │
    │       ├── TODO-032 (PDF billback extraction Cloud Fn) ← NEW INFRA
    │       │       │
    │       │       ├── TODO-033 (wine entity)
    │       │       └── TODO-034 (billback transforms)
    │       │
